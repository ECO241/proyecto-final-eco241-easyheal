...
const formulasController = {
  getAllformulas: async (req, res) => {
    ...
    try {
        const id = req.params.id; // Obtener el ID de la fórmula de los parámetros de la solicitud
        const data = await formulasService.getFormulasById(id); // Usar el método corregido para obtener la fórmula por ID
        res.json({ success: true, data });
    } catch (error) {
        console.error("Error retrieving data from Supabase:", error.message);
        res.status(500).json({ success: false, error: "Internal Server Error" });
